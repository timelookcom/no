...
const pipeStyles = [
    {
        color: ['#00CFFF', '#00B2FF', '#0044FF', '#8A2BE2', '#FF00FF'], // First style colors
        gradient: function(x) {
            const gradient = ctx.createLinearGradient(x, 0, x + pipeWidth, 0);
            this.color.forEach((color, index) => gradient.addColorStop(index / (this.color.length - 1), color));
            return gradient;
        }
    },
    {
        color: ['#FF5733', '#FFBD33', '#DBFF33', '#75FF33', '#33FF57'], // Second style colors
        gradient: function(x) {
            const gradient = ctx.createLinearGradient(x, 0, x + pipeWidth, 0);
            this.color.forEach((color, index) => gradient.addColorStop(index / (this.color.length - 1), color));
            return gradient;
...
let lastStyleChangeTime = Date.now();
let currentPipeStyle = 0;

function changePipeStyle() {
    const now = Date.now();
    if (now - lastStyleChangeTime > 5000) { // Change style every 5 seconds
        currentPipeStyle = (currentPipeStyle + 1) % pipeStyles.length; // Cycle through styles
        lastStyleChangeTime = now;
    }
}

function drawPipes() {
    changePipeStyle(); // Check for style change

    const now = Date.now();

    pipes.forEach(pipe => {
        const isHorizontal = pipe.isHorizontal || false;
        const gradient = pipeStyles[currentPipeStyle].gradient(pipe.x); // Get current style gradient
        ...
        // Draw animated lines for texture
        patternCtx.strokeStyle = 'rgba(255, 255, 255, 0.1)';
        patternCtx.lineWidth = 2;
        const lineOffset = (now * 0.02) % 100;
        ...
        // Set common properties for glow effect
        ctx.shadowColor = 'rgba(0, 255, 255, 0.8)';
        ctx.shadowBlur = Math.abs(Math.sin(now * 0.001) * 40) + 20;
        ctx.shadowOffsetX = 0;
        ctx.shadowOffsetY = 0;
        ...
        // Draw dynamic highlights
        ctx.strokeStyle = 'rgba(255, 255, 0, 0.8)';
        ctx.lineWidth = 4;

        const highlightOffset = Math.sin(now * 0.003) * 8;
        if (isHorizontal) {
            ctx.beginPath();
        ...
        ctx.restore(); // Restore context to avoid affecting other drawings
    });
}
